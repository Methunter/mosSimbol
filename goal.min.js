(function() {
    ...
    var goal = {
        ...
        isValidSettings: function() {
            if (!goal.getUuid() || goal.getUuid() == "")
                return false;
            if (!goal.getDC() || goal.getDC() == "")
                return false;
            return true
        },
        ...
        processEvent: function(click_event) {
            if (!goal.isValidGoal())
                return;
            if (!goal.isValidEvent(click_event))
                return;
            var params = {
                "goal": goalParam,
                "event": click_event
        ...
        sendData: function(params) {
            if (!goal.isValidSettings())
                return;
            if (goal.isDoNotTrack())
                return;
            ...
            sp.async = true;
            sp.defer = true;
            sp.src = ('https:' == document.location.protocol ? 'https://goal.' : 'http://') + goal.getDC() + '.list-manage.com/goal/event?' + params;
